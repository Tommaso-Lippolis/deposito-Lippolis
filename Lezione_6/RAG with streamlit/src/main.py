...
st.session_state.vector_store = VectorStore( embedding_model="text-embedding-ada-002")
...
def build_and_rerun():
    ...
    if st.session_state.vector_store.vector_store is None:
        st.warning("Carica almeno un documento e crea il Vector Store prima di usare il chatbot.")
    else:
...
if uploaded_file:
    ...
    st.button("Crea Vector Store", on_click=build_and_rerun())
